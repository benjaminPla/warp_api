...
pub async fn create_user(pool: Pool<Postgres>, email:String, password: String) -> Result<Json, warp::Rejection> {
    let row = sqlx::query("INSERT INTO users (email, password) VALUES ($1, $2) RETURNING id, email;").bind(email).bind(password).fetch_one(&pool).await.expect("Failed inserting user");
    let id: i32 = row.try_get("id").expect("Failed to try_get id");
    let email:String= row.try_get("email").expect("Failed to try_get email");
    let user = json!({
        "id": id,
